...
class NotificationService:
    ...
    @classmethod
    async def get_notification(cls, time: str) -> timedelta | None:
    ...
    async def delete_notification(
        ...
        async with uow:
            await uow.notification.delete(scheduler_id=scheduler_id)
            await uow.commit()
        scheduler.remove_job(job_id=scheduler_id)
    ...
    async def delete_many_notifications(
        ...
        async with uow:
            notifications = await uow.notification.find_many(task_id=task_id)
            await uow.notification.delete(task_id=task_id)
            await uow.commit()
        for notification in notifications:
